...
void forth(){
    ...
    cin >> a;
    long double x=12*pow(a,2) + 7*a -12;
    cout << "x=" << (long long int) lround(x) <<endl;
    cout << "Number 'x' for 4th task(part b): ";
    cin >> xy;
    long double y=3*pow(xy,3)+4*pow(xy,2)-11*xy+1;
    cout << "y=" << (long long int) lround(y) <<endl;
}
double fifth(){
    long double v,m;
